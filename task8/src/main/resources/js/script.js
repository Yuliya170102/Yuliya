...
let validateAd = (function(adItem){
    if (typeof(adItem.id) == 'string' &&
    typeof(adItem.description) == 'string' && adItem.description.length < 200  &&
    adItem.createdAt instanceof Date &&  
    typeof(adItem.link) == 'string' &&
    typeof(adItem.vendor) == 'string' && adItem.vendor.length != 0 &&
    adItem.hashTags instanceof Array &&
    typeof(adItem.discount) == 'string' &&  adItem.discount.length != 0 &&
    adItem.validUntil instanceof Date){
        return true;
    }else{
        return false;
    }
...
let addAd = (function(adItem){
    if (adList.findIndex(ad => ad.id === adItem.id) >= 0) {
        return false;
    }
